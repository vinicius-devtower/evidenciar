{{--
    Template "Evidenciar Institucional" v1 — Renderização pública/preview.

    Recebe (via SiteBuilderService / PublicSiteController):
      - $site    (App\Models\Site)
      - $content (array JSON salvo em sites.content)

    Este template NÃO duplica HTML/CSS/JS da landing: ele reaproveita
    diretamente as mesmas partials/sections/modais de resources/views/landing,
    que já sabem ler $content quando presente e caem no texto padrão da
    Evidenciar quando ausente (uso pela LandingController estática).

    $plans é injetado aqui porque, nesse fluxo de renderização, ninguém mais
    passa essa variável (ao contrário da LandingController, que já injeta).
--}}
@php
